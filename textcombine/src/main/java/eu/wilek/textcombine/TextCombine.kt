...
data class TextCombine(val texts: List<TextValue>, val spans: List<StyleSpan> = emptyList()) {
    ...
    sealed class DimensionValue {
        data class FromPx(@Dimension(unit = Dimension.PX) val value: Float) : DimensionValue()
        data class FromDp(@Dimension(unit = Dimension.DP) val value: Float) : DimensionValue()
